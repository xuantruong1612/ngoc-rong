...
public class MapMaBu {
    ...
    public static final byte HOUR_OPEN_MAP_THANHDIA = 20;
    public static final byte MIN_OPEN_MAP_THANHDIA = 59;
    public static final byte SECOND_OPEN_MAP_THANHDIA = 59;
    
    public static final byte HOUR_CLOSE_MAP_THANHDIA = 23;
    public static final byte MIN_CLOSE_MAP_THANHDIA = 59;
    public static final byte SECOND_CLOSE_MAP_THANHDIA = 59; 
    
    
    
    

    public static final byte HOUR_OPEN_MAP_TN2 = 5;
    public static final byte MIN_OPEN_MAP_TN2 = 59;
    public static final byte SECOND_OPEN_MAP_TN2 = 59;
    public static final byte HOUR_CLOSE_MAP_TN2 = 6;
    ...
    public static long TIME_OPEN_THANHDIA;
    public static long TIME_CLOSE_THANHDIA;
    ...
    public void setTimeJoinMapMaBu() {
        if (i.day == -1 || i.day != TimeUtil.getCurrDay()) {
            ...
            try {
                ...
                TIME_OPEN_THANHDIA = TimeUtil.getTime(TimeUtil.getTimeNow("dd/MM/yyyy") + " " + HOUR_OPEN_MAP_THANHDIA + ":" + MIN_OPEN_MAP_THANHDIA + ":" + SECOND_OPEN_MAP_THANHDIA, "dd/MM/yyyy HH:mm:ss");
                TIME_CLOSE_THANHDIA = TimeUtil.getTime(TimeUtil.getTimeNow("dd/MM/yyyy") + " " + HOUR_CLOSE_MAP_THANHDIA + ":" + MIN_CLOSE_MAP_THANHDIA+ ":" + SECOND_CLOSE_MAP_THANHDIA, "dd/MM/yyyy HH:mm:ss");
...
//    }
    private void kickOutOfMapPVP(Player player) {
        if (player.zone.map.mapId == 145) {
            Service.getInstance().sendThongBao(player, "Thời gian giải đã kết thúc, tàu vận chuyển sẽ đưa bạn về nhà");
            ChangeMapService.gI().changeMapBySpaceShip(player, player.gender + 21, -1, 250);
        }
    }
  private void kickOutOfMapgiatoc(Player player) {
        if (player.zone.map.mapId == 212) {
            Service.getInstance().sendThongBao(player, "Thời gian Thánh Địa đã kết thúc, tàu vận chuyển sẽ đưa bạn về nhà");
            ChangeMapService.gI().changeMapBySpaceShip(player, player.gender + 21, -1, 250);
        }
    }   
...
//    }
    ...
    private void ketthucPVP(Player player) {
     ...
     private void ketthucgiatoc(Player player) {
        player.zone.finishMapthanhdia = true;
        List<Player> playersMap = player.zone.getPlayers();
        for (int i = playersMap.size() - 1; i >= 0; i--) {
            Player pl = playersMap.get(i);
            kickOutOfMapgiatoc(pl);
        }
    }  
...
//                }
              ...
              if (now < TIME_OPEN_THANHDIA || now > TIME_CLOSE_THANHDIA) {
                    ketthucgiatoc(player);
                }   
