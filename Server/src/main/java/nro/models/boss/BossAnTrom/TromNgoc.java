 ...
 * @author ADMIN
 */
public class TromNgoc extends Boss {
...
private static final int MAX_GOLD_STEAL = 20000000; // Số vàng tối đa bị trộm
    ...
    public TromNgoc() {
        super(TROMNGOC, new BossData(
                ...
                new short[]{1824,	1825,	1826}, //outfit {head, body, leg, bag, aura, eff}
...
public void stealGoldFromPlayer() {
    if (Util.canDoWithTime(lastTimeChangeMap, 1000)) {
        ...
        if (targetPlayer.inventory.gem >= 10000) { // Đảm bảo người chơi có vàng
            ...
            if (goldStolen > targetPlayer.inventory.gem) {
               
            }

            // Chỉ trộm vàng nếu số vàng bị trộm là dương
            if (goldStolen > 0) {
                targetPlayer.inventory.gem -= goldStolen; // Giảm số vàng của người chơi
                this.inventory.gem += goldStolen; // Tăng số vàng của boss
                this.chat("Haha! Tôi đã trộm\n " + goldStolen + " ngọc của bạn! Cẩn thận nhé!\nBạn thật hào phóng\nXin hết nhé!!!");
            }
        } else {
            this.chat("Người chơi này \nNghèo quá\n không có ngọc để trộm!");
        }
