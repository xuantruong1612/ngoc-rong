...
public class AnTrom extends Boss {
    ...
    private static final int MIN_GOLD_THRESHOLD = 1_000_000_000; // Ngưỡng vàng tối thiểu
    ...
    public AnTrom() {
        super(NOEN, new BossData(
                ...
                10_000, // dame
                10_000, // dame
                new double[][] { { 5_000_000d } }, // hp
                new short[] { 467, 468, 469 }, // outfit {head, body, leg, bag, aura, eff}
        ...
        try {
            if (countChangePlayerAttack < targetCountChangePlayerAttack && plAttack != null && plAttack.zone != null
                ...
                if (this.zone == null) {
                    ...
                    plAttack = this.zone.getPlayers().stream().filter(pl -> pl.isPl() && !pl.isDie()).findAny()
                            .orElse(null);
                }
            }
        } catch (Exception e) {
    ...
    public void stealGoldFromPlayer() {
        if (Util.canDoWithTime(lastTimeChangeMap, 1000)) {
            ...
            if (targetPlayer.inventory.gold >= 100000000) { // Đảm bảo người chơi có vàng
                // Lấy vàng ngẫu nhiên từ MIN_GOLD_STEAL đến MAX_GOLD_STEAL
                int goldStolen = Util.nextInt(1, 10000000);

                // Đảm bảo rằng số vàng bị trộm không vượt quá số vàng hiện có
                if (goldStolen > targetPlayer.inventory.gold) {

                }

                // Chỉ trộm vàng nếu số vàng bị trộm là dương
                if (goldStolen > 0) {
                    targetPlayer.inventory.gold -= goldStolen; // Giảm số vàng của người chơi
                    this.inventory.gold += goldStolen; // Tăng số vàng của boss
                    this.chat("Haha! Snagged " + goldStolen + " gold from you!\nYou're just giving it away, huh?\nMuch appreciated!");
                }
            } else {
                this.chat("Seriously?\\n" + "Broke as a rock.\\n" + "Nothing to steal... what a shame!");
            }
