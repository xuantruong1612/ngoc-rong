...
public class EffectSkin {
    ...
    public void update() {
        ...
        if (!this.player.isBoss && !this.player.isPet && !this.player.isClone) {
            updateTrainArmor();
        }
        ...
        try {
            if (this.player.nPoint.wearingNezuko) {
                if (Util.canDoWithTime(lastTimeEffNezuko, 120000)) {
                    Service.getInstance().chat(this.player, "Tiến vào trạng thái điên loạn");
    ...
    private void updateCTHaiTac() {
        if (this.player.setClothes.ctHaiTac != -1
            ...
            for (Player pl : players) {
                ...
                if (!pl.isPet && !pl.isClone && Util.canDoWithTime(lastTimeUpdateCTHT, 5000)) {
