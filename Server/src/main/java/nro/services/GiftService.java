...
public class GiftService {
    ...
    public void use(Player player, String code) {
        ...
        try {
            ...
            try {
                if (!res.first()) {
                    Service.getInstance().sendThongBaoOK(player, "GIFTCODE ĐÃ BỊ XÓA VUI LÒNG NHẬP CODE VIP666,VIP777,VIP888");
                    return;
                }
                ...
                if (status == 1) {
                    ...
                    Service.getInstance().sendThongBaoFromAdmin(player, "Cần kích hoạt Dấu Ấn Rồng\n FREE GD ALL Vật Phẩm\nNhập Code Vip FREE Hằng Ngày\nHoặc Đạt VIP 6\n"
                            + "Hoặc Có Thể Chơi Sever Khác");
                    return;
                }
    ...
    public static void danhsachgiftcode(Player player) {
        try {
            PreparedStatement stmt = DBService.gI().getConnectionForGame().prepareStatement("SELECT code FROM `gift_codes` WHERE status = 0");
            ResultSet res = stmt.executeQuery();
            int index = 1;
            StringBuilder message = new StringBuilder("Danh sách giftcode:\n");
            while (res.next()) {
                String code = res.getString("code");
                message.append(index).append(". ").append(code).append("\n");
                index++;
            }
            NpcService.gI().createMenuConMeo(player, 0, 0, message.toString(), "Ok");  // Hiển thị thông báo chung
