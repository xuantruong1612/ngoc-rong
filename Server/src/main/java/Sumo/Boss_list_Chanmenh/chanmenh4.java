 ...
 *
 */
public class chanmenh4 extends FutureBoss {

    public chanmenh4() {
        super(BossFactory.chanmenh4, BossData.chanmenh4);
    }
    ...
    public void rewards(Player pl) {
        ...
        if (Util.isTrue(50, 100)) { // Tỉ lệ 50%
            ...
            for (int i = 0; i < slcarot3; i++) {
                ItemMap carot = new ItemMap(zone, 1359, 1, 10 * i + this.location.x,
                        zone.map.yPhysicInTop(this.location.x, 0), -1);
              carot.options.add(new ItemOption(50, Util.nextInt(10, 400)));
                carot.options.add(new ItemOption(77, Util.nextInt(10, 400)));
                carot.options.add(new ItemOption(103, Util.nextInt(10, 400)));
