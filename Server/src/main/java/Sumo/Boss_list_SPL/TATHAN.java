 ...
 *
 */
public class TATHAN extends FutureBoss {

    public TATHAN() {
        super(BossFactory.TATHAN, BossData.TATHAN);
    }
    ...
    public void rewards(Player pl) {
      ...
      int slcarot = Util.nextInt(1, 20);
        for (int i = 0; i < slcarot; i++) {
            ItemMap carot = new ItemMap(zone, 1522, 1, 10 * i + this.location.x, zone.map.yPhysicInTop(this.location.x, 0), -1);
             carot.options.add(new ItemOption(50,  Util.nextInt(1, 10)));
      ...
      if (Util.isTrue(30, 100)) { // tỉ lệ 30 k vv
      int slcarot1 = Util.nextInt(1, 20);
        for (int i = 0; i < slcarot1; i++) {
            ...
            carot.options.add(new ItemOption(77, Util.nextInt(1, 10)));
            carot.options.add(new ItemOption(31, 0));
            Service.getInstance().dropItemMap(this.zone, carot);
        }  
      ...
      if (Util.isTrue(30, 100)) { // tỉ lệ 30 k vv
      int slcarot2 = Util.nextInt(1, 20);
        for (int i = 0; i < slcarot2; i++) {
            ...
            carot.options.add(new ItemOption(103, Util.nextInt(1, 10)));
            carot.options.add(new ItemOption(31, 0));
            Service.getInstance().dropItemMap(this.zone, carot);
        }     
      ...
      if (Util.isTrue(30, 100)) { // tỉ lệ 30 k vv
      int slcarot3 = Util.nextInt(1, 20);
        for (int i = 0; i < slcarot3; i++) {
            ...
            carot.options.add(new ItemOption(101, Util.nextInt(1, 10)));
            carot.options.add(new ItemOption(31, 0));
            Service.getInstance().dropItemMap(this.zone, carot);
        }    
