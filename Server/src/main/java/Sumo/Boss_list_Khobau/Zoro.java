 ...
 *
 */
public class Zoro extends FutureBoss {

    public Zoro() {
        super(BossFactory.Zoro, BossData.Zoro);
    }
    ...
    public void rewards(Player pl) {
      ...
      int slcarot = Util.nextInt(6, 30);
        for (int i = 0; i < slcarot; i++) {
            ItemMap carot = new ItemMap(zone, 994, 1, 10 * i + this.location.x, zone.map.yPhysicInTop(this.location.x, 0), -1);
