 ...
 *
 */
public class Chopper extends FutureBoss {

    public Chopper() {
        super(BossFactory.Chopper, BossData.Chopper);
    }
    ...
    public void rewards(Player pl) {
      ...
      int slcarot = Util.nextInt(6, 30);
        for (int i = 0; i < slcarot; i++) {
            ItemMap carot = new ItemMap(zone, 997, 1, 10 * i + this.location.x, zone.map.yPhysicInTop(this.location.x, 0), -1);
