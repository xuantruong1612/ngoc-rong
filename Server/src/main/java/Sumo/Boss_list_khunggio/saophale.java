package Sumo.Boss_list_khunggio;
 ...
 */
public class saophale extends FutureBoss {
    ...
    public saophale() {
        super(BossFactory.saophale, BossData.saophale);
    }
    ...
    public void rewards(Player pl) {
        ...
        if (Util.isTrue(2, 100)) { // Tỉ lệ 50%
            ...
            for (int i = 0; i < slcarot3; i++) {
                ItemMap carot = new ItemMap(zone, 1522, 1, 10 * i + this.location.x,
                        zone.map.yPhysicInTop(this.location.x, 0), -1);
              carot.options.add(new ItemOption(50, Util.nextInt(10, 100)));
             ...
             if (Util.isTrue(30, 100)) { // Tỉ lệ 50%
            int slcarot4 = 1;
            for (int i = 0; i < slcarot4; i++) {
                ItemMap carot1 = new ItemMap(zone, 1523, 1, 10 * i + this.location.x,
                        zone.map.yPhysicInTop(this.location.x, 0), -1);
              carot1.options.add(new ItemOption(77, Util.nextInt(10, 100)));
                carot1.options.add(new ItemOption(31, 0));
                // Tỉ lệ 99% ra opti 93
                if (Util.isTrue(998, 1000)) {
                    carot1.options.add(new ItemOption(93, 1));
                }
               ...
               if (Util.isTrue(60, 100)) { // Tỉ lệ 50%
            int slcarot3 = 1;
            for (int i = 0; i < slcarot3; i++) {
                ItemMap carot = new ItemMap(zone, 1524, 1, 10 * i + this.location.x,
                        zone.map.yPhysicInTop(this.location.x, 0), -1);
              carot.options.add(new ItemOption(103, Util.nextInt(10, 100)));
                carot.options.add(new ItemOption(31, 0));
                // Tỉ lệ 99% ra opti 93
                if (Util.isTrue(998, 1000)) {
                    carot.options.add(new ItemOption(93, 1));
