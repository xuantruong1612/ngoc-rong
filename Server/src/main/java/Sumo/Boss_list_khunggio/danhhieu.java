 ...
 */
public class danhhieu extends FutureBoss {
    ...
    public danhhieu() {
        super(BossFactory.danhhieu, BossData.danhhieu);
    }
  ...
  @Override
    public void rewards(Player pl) {
        ...
        if (Util.isTrue(2, 100)) { // Tỉ lệ 50%
            ...
            for (int i = 0; i < slcarot3; i++) {
                ItemMap carot = new ItemMap(zone, Util.nextInt(1710, 1718), 1, 10 * i + this.location.x,
                        zone.map.yPhysicInTop(this.location.x, 0), -1);
              carot.options.add(new ItemOption(50, Util.nextInt(10, 100)));
              carot.options.add(new ItemOption(77, Util.nextInt(10, 100)));
              carot.options.add(new ItemOption(103, Util.nextInt(10, 100)));
              carot.options.add(new ItemOption(0, Util.nextInt(10, 10000)));
   ...
   @Override
    public void update() {
       if (!isBossSummoned && TimeUtil.getCurrHour() == 07) {
            this.joinMap();
            isBossSummoned = true;
     }
    }
